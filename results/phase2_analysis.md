...
7. **规模有限**: 5 域、125 复杂度上限。更大规模（10+ 域、500+ 复杂度）下 rewrite 的可扩展性未知

### 7.4 验证边界的诚实界定

两阶段实验验证了核心 thesis 的**必要条件**——在构造的最小验证场景中，纯扩展确实退化为规则堆积，rewrite 确实带来系统级收益。但这还不是**充分验证**：

- 跨域任务是**反向设计**的（先分析 EO 的漏洞，再构造触发任务），能证明"漏洞存在且可修复"，但不能推断自然任务分布中的退化概率
- 0% vs 100% 的**断崖式分离**过于极端，真实场景更可能是梯度退化，rewrite 在灰色地带的效果尚未检验
- 仅测试了 **Type A（同义谓词不可达）** 这一种失败模式，Type B（约束冲突）和 Type C（搜索空间爆炸）未覆盖

这些结果足以支撑 tech note 或 workshop paper 级别的 claim，但距离顶会论文的验证标准还需要自然任务分布和更大规模的实验数据。
