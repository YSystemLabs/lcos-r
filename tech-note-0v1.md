...
本节保留实验设计阶段的原始预期，但按当前实验结果补充其验证状态，避免与后文 9.6、9.7 的审计结论混淆。

#### 原始预期 1：Baseline 只能覆盖初始域

- **原始预期**：Baseline 在家居领域内性能最优，但无法处理后续领域引入的新语义
- **当前状态**：**已验证**。Phase 1 中 Baseline 在 S1/S3/S4 的可规划率降至 0%，S2 为 50%，说明不扩展时无法稳定覆盖新域任务；见 9.6。

#### 原始预期 2：Expand-Only 会随着扩展出现复杂度和可规划性双重退化

- **原始预期**：Expand-Only 随领域累积，谓词数和规则数快速上升（线性或超线性），跨领域规则冲突增加，可规划率下降
- **当前状态**：**部分验证**。
  - “复杂度快速上升”已验证：Phase 1 中 Expand-Only 的复杂度从 59 增长到 125，增长 112%；见 9.6。
  - “单域任务上的可规划率下降”**未验证**：在当前 Phase 1 的单域、短程采样任务上，Expand-Only 仍为 100%，说明这批任务不足以暴露跨域规划退化；该问题已由 9.7 的跨域组合任务补强验证。
  - “规则冲突增加”当前**未单独量化报告**，因此主文档不把它作为已完成证据线。

#### 原始预期 3：Expand+Rewrite 会抑制复杂度增长并维持可规划性

- **原始预期**：在吸收同样新语义的前提下，复杂度增长被显著抑制；跨领域同义谓词被合并，冗余规则被折叠，可规划率保持稳定
- **当前状态**：**大体已验证，但需区分 Phase 1 与 Phase 2 的职责边界**。
  - 在 Phase 1 中，“复杂度增长被显著抑制”已验证：Expand+Rewrite 在 S4 的复杂度为 115，显著低于 Expand-Only 的 125，配对 t 检验 $p = 0.0248$；见 9.6。
  - 在 Phase 1 中，“单域任务上的可规划率保持稳定”也成立，但这只能说明当前单域采样任务可被正常求解，**不能单独推出**“rewrite 已保护跨域可规划性”；该部分直接证据由 9.7 提供。
  - 在 Phase 2 中，“跨域失败模式上的可规划性恢复”已验证：在当前 30 个跨域 A/B/C 任务上，EO 为 0%，ER 为 100%，McNemar exact test 为 $p = 1.86 \times 10^{-9}$；见 9.7。

#### 原始预期 4：OOD 泛化任务中，Expand+Rewrite 将优于 Expand-Only

- **原始预期**：Expand+Rewrite 在跨领域复合场景上的可规划率显著高于 Expand-Only，因为更紧凑的表示减少了规则冲突
- **当前状态**：**未按 9.3.1 的原始 OOD 复合场景单独执行并报告**。
  - 当前已经完成的是 9.7 的跨域组合任务验证，它提供了“当前已实现跨域失败模式”上的直接证据。
  - 但它不等同于“自然 OOD 复合场景分布上的泛化结论”，因此本 note 目前不把 OOD 泛化写成已完成结论。

因此，到当前版本为止，这篇 note 已经拿到两条证据线：

1. **Phase 1**：验证了 rewrite 对表示复杂度增长的显著抑制
2. **Phase 2**：验证了 rewrite 对当前构造的跨域失败模式的可规划性恢复

而“自然 OOD 复合场景上的一般性泛化优势”仍属于后续待补的证据。

### 9.6 实验结果（基于 AgiBot World Task Catalog 的验证实验）

以下为实际实验结果。实验配置：5 阶段渐进扩展 × 4 组系统 × 30 任务/领域。

**审计口径说明**：本节只报告 Phase 1 在当前单域采样任务分布下能够稳妥支持的结果。换言之，9.6 的职责是验证“扩展是否带来表示复杂度增长，以及 rewrite 是否显著抑制这种增长”；它**不单独承担**“跨域可规划性保护”的证明任务，该部分证据由 §9.7 提供。
...
- **Expand+Rewrite 复杂度增长**：59 → 115（增长 95%；相对 Expand-Only 少 10 个语义要素，S4 相对复杂度削减 8.0%，增长幅度低 17 个百分点）
...
- **Pass 3 逐任务验证率**：S1-S4 的 Expand+Rewrite 均为 100%
- **当前解释边界**：对可规划系统而言，Phase 1 的高可规划率主要说明“当前单域采样任务可以被当前签名和 planner 正常求解”，不说明“Expand-Only 在跨域任务上不会退化”

#### Rewrite Pass 分解

- **Pass 1（谓词消除）**：每阶段检测并消除 2 个同义谓词（如 `on_shelf` → `on`，`in_cart` → `in`）
- **Pass 2（规则折叠）**：每阶段折叠 1 条同构推理规则
- **Pass 3（对象裁剪）**：per-task 裁剪无关领域类型，并在单任务 `Q_t^{task}` 变体验证下保持当前任务可规划性

#### 与预期对比

1. ✓ Baseline 在新领域可规划率降至 0%（符合预期）
2. ✓ Expand-Only 复杂度线性增长 112%（符合"线性或超线性"预期）
3. ✓ Expand+Rewrite 在复杂度上显著优于 Expand-Only（ $p = 0.0248$ ），因此“rewrite 抑制表示膨胀”这一点成立
4. ✓ Expand+Rewrite 与 ManualOpt 效果接近（ $p = 0.0577$ 无显著差异），说明自动 rewrite 在当前任务域内接近人工优化水平
5. ✓ Pass 3 已按任务级验证执行，S1-S4 的逐任务验证率为 100%
6. △ Expand-Only 的可规划率在本阶段未下降（100%），原因是采样任务均为单域、单步或两步简单任务，不涉及跨领域冲突；因此 9.6 **不能**单独推出“Expand-Only 不会发生规划退化”，这一问题已转交 §9.7 的跨域任务验证
...
4. 渐变谓词以布尔近似处理
5. 任务分布偏向单域、短程任务，因此这部分结果主要适合评估表示复杂度，而不适合单独评估跨域可规划性保护

#### 一阶段结论边界

Phase 1 到这里为止，能够稳妥支持的结论是：

1. 五域渐进扩展会带来表示复杂度持续增长。
2. rewrite 在当前任务分布下能显著抑制这种复杂度增长。
3. Baseline 不扩展时无法覆盖新域任务。
4. 自动 rewrite 的压缩效果接近当前人工优化基线。

Phase 1 **不能单独支持**以下更强表述：

1. "rewrite 已一般性地保护了跨域可规划性"
2. "Expand-Only 会在自然任务分布上普遍出现规划退化"
3. "开放扩展若不 rewrite，一定会在所有任务类型上退化"

原因是本阶段任务主要是单域、单步或两步模板任务，适合验证"表示是否膨胀"，不适合单独验证"跨域规划是否退化"。因此，Phase 1 应被理解为：

> **rewrite 对复杂度抑制的必要证据，而不是对跨域可规划性保护的充分证据。**

### 9.7 二阶段实验：跨域组合任务的可规划性验证

一阶段实验（§9.6）验证了 rewrite 对复杂度的抑制效果，但由于采样任务为单域、单步或两步短程任务，**未能暴露 Expand-Only 的可规划率退化**：在当前 Phase 1 任务分布下，Expand-Only / Expand+Rewrite / Expand+ManualOpt 的可规划率都保持在 100%。为此，二阶段补强为 30 个**跨域组合任务**，覆盖三类可复现的失败模式：Type A（目标别名不可达）、Type B（动作前置条件接续失败）、Type C（高干扰单目标搜索压力）。

#### 跨域任务设计原理

核心机制：在 Expand-Only 的 sigma 中存在 7 对同义谓词（如 `on_shelf`↔`on`、`on_tray`↔`on`、`on_desk`↔`on`、`on_belt`↔`on`），这些谓词在 Expand+Rewrite 中被合并为基础谓词。当一个跨域任务的 goal 或关键动作前置条件必须跨越这些同义谓词边界时，Expand-Only 会因谓词名不匹配而失败。

**失败模式示例**：

- Type A：`place(food, tray)` 产生 `on(food, tray)`，但 `serve(food, tray)` 在 EO 中需要 `on_tray(food, tray)` → 匹配失败。
- Type B：任务初始事实给出 `on_shelf(food, shelf)` 或 `on_desk(food, desk)`，而 `serve(food, tray)` 的前置条件在 EO 中仍是 `on_tray(food, tray)`；ER 中这些位置谓词统一成 `on` 后即可直接接续。
- Type C：在 Type A/B 的别名断裂之上再加入额外干扰对象，形成更高 grounding 分支数的单目标压力任务，检验 rewrite 是否仍能在受限预算下恢复可规划性。

30 个任务模板在 Stage 4 下分布为：18 个 Type A、6 个 Type B、6 个 Type C。

| 类型 | 任务数 | 当前实现中的含义 | 代表模板 |
|------|--------|-------------------|----------|
| A | 18 | goal-side synonym gap | `xd_form_to_desk`, `xd_cup_to_tray` |
| B | 6 | action-precondition handoff gap | `xd_shelf_serve_fruit`, `xd_desk_serve_salad` |
| C | 6 | high-branching single-goal stress | `xd_shelf_to_desk_crowded`, `xd_belt_to_tray_crowded` |
...
| 系统 | standard | relaxed |
|------|----------|---------|
| Baseline | 0% (0/30) | 0% (0/30) |
| Expand-Only | **0% (0/30)** | **0% (0/30)** |
| Expand+Rewrite | 77% (23/30) | **100% (30/30)** |
| Expand+ManualOpt | 77% (23/30) | 100% (30/30) |
...
| S3 (+Restaurant, 20 tasks) | 0% | 100% | 100% |
| S4 (+Office, 30 tasks) | 0% | 100% | 100% |

#### 统计检验

- **McNemar exact test**（EO vs ER, Stage 4, relaxed）： $p = 1.86 \times 10^{-9}$ （极显著）
- **风险差**： $\Delta = 1.0000$ （EO 0% → ER 100%，完全分离）
- **95% CI**： $[1.0000, 1.0000]$ （所有 30 个任务一致表现为 EO 失败 + ER 成功）
- **预算敏感性**：Stage 4 下 ER 在 standard 为 77%（23/30），在 relaxed 为 100%（30/30）；EO 在两组预算下都为 0%，说明当前主导失败机制仍是谓词表示失配，而 relaxed 预算主要负责补足 Type C 高干扰任务
...
| Expand+Rewrite | 40,051 | 188,197 |

ER 平均搜索节点为 EO 节点预算的 21%，说明紧凑表示不仅恢复可规划性，还降低了搜索代价。
...
| EO 可规划率 | 100% | **0%** |
| ER 可规划率 | 100% | **100%** |
| ER vs EO 差异 | 无法区分 | $p = 1.86 \times 10^{-9}$ |
| ER ≈ ManualOpt | $p = 0.0577$（一阶段复杂度） | 二阶段结果一致 |

#### 结论

二阶段实验验证了核心假说中一阶段未能触及的关键预期：

1. ✓ **Expand-Only 可规划率在 30 个 A/B/C 任务上退化至 0%**——这说明别名裂解既能表现为 goal 不可达，也能表现为动作前置条件接续失败
2. ✓ **Expand+Rewrite 完全恢复可规划性至 100%**——在 relaxed 预算下，rewrite 消除了这些跨域别名断裂
3. ✓ **自动 rewrite = 人工优化**——两者在所有 30 个任务上结果完全一致
4. **综合两阶段**：rewrite 既抑制复杂度增长（一阶段, $p = 0.0248$ ），又在当前构造的跨域 A/B/C 任务上恢复可规划性（二阶段, $p = 1.86 \times 10^{-9}$ ）

#### 未验证的边界

1. **任务仍是反向设计**：30 个跨域任务依旧基于 EO 中已知的别名裂解弱点构造，能证明"漏洞存在且 rewrite 可修复"，但不能说明自然任务分布下此类退化出现的频率
2. **Type B 不是约束推理冲突**：由于当前 planner 不消费 C_t，现阶段 Type B 落在动作前置条件接续失败，而不是路线图早期设想的显式约束冲突
3. **Type C 仍不是多目标深链**：当前 Type C 实现为高干扰单目标搜索压力任务，尚未覆盖真正的多目标深链节点爆炸
4. **退化仍是断崖式而非梯度式**：0% vs 100% 的完全分离是极端情况，真实场景中更可能出现渐进退化（如 80% → 50%）
5. **规模有限**：5 域、125 复杂度上限。更大规模（10+ 域、500+ 复杂度）下 rewrite 的可扩展性未知
6. **仍在符号层**：实验在 PDDL 表示层面进行，未涉及感知噪声、执行失败、连续动作参数等物理级因素
...
Sections 9.6–9.7 的实验为上述数学框架提供了初步经验支撑：

- **rewrite legality**：每次 rewrite pass 执行后均通过 $Q_\star$ （10/10 项）回归测试；其中 Pass 3 已接入单任务 $Q_t^{task}$ 变体验证，确认任务级裁剪不会破坏当前任务可规划性；二阶段跨域任务进一步表明 rewrite 后的系统在当前 A/B/C 失败模式下仍能保持可规划性恢复
...
>
> **Phase 1** 提供的是 rewrite 对复杂度抑制的必要证据：一阶段实验显示 Expand-Only 与 Expand+Rewrite 在表示复杂度上出现统计显著差异（ $p = 0.0248$ ，Cohen's $d = 2.42$ ），说明持续扩展后的 rewrite 确实能压缩内部表示。
>
> **Phase 2** 进一步提供跨域可规划性证据：二阶段实验在 30 个专门构造的跨域 A/B/C 任务上得到 EO 0% vs ER 100%（ $p = 1.86 \times 10^{-9}$ ），说明在当前已实现的别名裂解失败模式下，rewrite 能恢复跨域可规划性。
>
> 因而更准确的综合表述是：Phase 1 证明了"rewrite 抑制复杂度增长"，Phase 2 证明了"在当前验证任务上，rewrite 能修复一类具体跨域规划退化"；两者合起来支持本文 thesis，但仍不足以推出对开放世界自然任务分布的一般性充分结论。
...
> **实验实测参考值（S4 阶段，5 域合并后 Expand+Rewrite）**：全局类型数 94，全局谓词数 49，全局动作数 45， $Q_\star$ 实测 10 项。per-task 裁剪（Pass 3）可将类型数降至 ≤10、谓词数 ≤15、动作数 ≤10。
...
- [x] 运行 Phase 1 的 5 阶段 × 4 组对比实验，并补做 Phase 2 的跨域组合任务实验
- [x] 完成 Phase 1 / Phase 2 的统计分析与可视化，并写回分析报告和主文档

后续优先级（基于实验发现）：

1. ~~**更复杂的任务集**~~：已完成（§9.7）。30 个跨域组合任务（18 个 A，6 个 B，6 个 C）验证了 Expand-Only 可规划率退化至 0%，Expand+Rewrite 恢复至 100%（ $p = 1.86 \times 10^{-9}$ ）
